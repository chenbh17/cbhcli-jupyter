...
export function stripAnsi(s) {
    return String(s !== null && s !== void 0 ? s : '').replace(/\x1b(?:\[[0-9;?]*[A-Za-z]|\][^\x07\x1b]*(?:\x07|\x1b\\)|\([0-9A-B]|[=>#][0-9]?)/g, '');
}
/** 复制文本到剪贴板（兼容非安全上下文）。 */
...
function collectEchartsBlocks(container) {
    ...
    container.querySelectorAll('pre code').forEach(code => {
        ...
        if (lang === 'echarts' || lang === 'echart') {
            ...
            if (looksLikeEcharts(parseEchartsOption(src)))
                blocks.push({ pre, src });
        }
    });
    return blocks;
...
async function renderMermaidSafe(mermaid, src) {
    var _a, _b;
    if (_mermaidSvgCache.has(src))
        return _mermaidSvgCache.get(src);
    const id = 'cbh-mmd-' + ++_diagSeq;
    let lastErr = null;
    for (let attempt = 0; attempt < 2; attempt++) {
        try {
            (_a = document.getElementById(id)) === null || _a === void 0 ? void 0 : _a.remove();
            (_b = document.getElementById('d' + id)) === null || _b === void 0 ? void 0 : _b.remove();
...
HL_KEYWORDS.python.push('print', 'len', 'range', 'str', 'int', 'float', 'list', 'dict', 'set', 'tuple', 'type', 'isinstance', 'enumerate', 'zip', 'map', 'filter', 'open', 'super');
HL_KEYWORDS.javascript.push('console', 'document', 'window', 'JSON', 'Math', 'Object', 'Array', 'String', 'Number', 'Promise', 'fetch', 'require', 'module', 'process');
const HL_BUILTINS = {
...
export function guessLang(filePath) {
    ...
    const ext = ((_a = String(filePath).split('.').pop()) === null || _a === void 0 ? void 0 : _a.toLowerCase()) || '';
...
export function codeBlockEl(code, lang, labelText) {
...
export function enhanceCodeBlocks(container) {
    container.querySelectorAll('pre').forEach(pre => {
        ...
        if (!pre.querySelector('.cbh-code-copy-btn')) {
            const codeText = ((_a = pre.querySelector('code')) === null || _a === void 0 ? void 0 : _a.textContent) || pre.textContent || '';
...
// ---------------------------------------------------------------------------
export function diffBlockEl(oldStr, newStr, lang, labelText) {
    const wrap = mkEl('div');
    if (labelText) {
        wrap.appendChild(mkEl('div', 'cbh-tool-code-label', labelText));
    }
    const block = mkEl('div', 'cbh-diff-block');
    const oldLines = String(oldStr !== null && oldStr !== void 0 ? oldStr : '').split('\n');
    const newLines = String(newStr !== null && newStr !== void 0 ? newStr : '').split('\n');
...
export function normalizeTodos(args) {
    var _a, _b, _c;
    ...
    for (const item of t) {
        if (typeof item === 'string') {
            ...
            const content = String((_c = (_b = (_a = item.content) !== null && _a !== void 0 ? _a : item.task) !== null && _b !== void 0 ? _b : item.title) !== null && _c !== void 0 ? _c : '');
            if (content) {
...
export function todoPanelEl(todos) {
...
// ---------------------------------------------------------------------------
export function renderToolArgs(container, name, args) {
    args = args || {};
    switch (name) {
        case 'Todo':
        ...
        case 'edit': {
            const fp = args.file_path || '';
            container.appendChild(diffBlockEl(args.old_str || '', args.new_str || '', guessLang(fp), `✏️ ${fp}`));
            break;
        }
        case 'read': {
            const fp = args.file_path || '';
        ...
        case 'grep':
            container.appendChild(codeBlockEl(`/${args.pattern || ''}/  in  ${args.path || '.'}`, null, '🔍 正则搜索'));
            break;
        case 'glob':
...
export function renderToolResult(container, name, preview, okFlag) {
