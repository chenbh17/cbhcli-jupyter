...
export class NotebookClient {
    ...
    async _editCell(params) {
        ...
        if (params.cell_type && params.cell_type !== cell.model.type) {
            try {
                (_b = (_a = cell.model.sharedModel).setCellType) === null || _b === void 0 ? void 0 : _b.call(_a, params.cell_type);
            }
            catch (_c) {
                /* cell 类型修改失败时忽略 */
            }
        }
        return ok(`已修改 cell ${index}${params.selection_text !== undefined ? '（替换片段）' : '（整体替换）'}`);
    }
    ...
    async _insertCell(params) {
        ...
        if (cellType === 'markdown') {
            try {
                (_b = (_a = newCell.model.sharedModel).setCellType) === null || _b === void 0 ? void 0 : _b.call(_a, 'markdown');
            }
            catch (_d) {
                /* ignore */
            }
        ...
        if (params.execute) {
            if (newCell instanceof CodeCell) {
                void this._executeCellInPlace(nb, newCell, String((_c = newCell.model.sharedModel.getSource()) !== null && _c !== void 0 ? _c : ''), 300000);
            }
        }
