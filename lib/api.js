...
export async function requestAPI(path, init) {
    ...
    if (!response.ok) {
        ...
        try {
            ...
            code = (data === null || data === void 0 ? void 0 : data.error) || '';
            serverMessage = (data === null || data === void 0 ? void 0 : data.message) || '';
        }
        catch (_a) {
            /* 保持 statusText */
        }
        ...
        if (response.status === 401 && code === 'not_logged_in') {
            const authErr = new ServerConnection.ResponseError(response, serverMessage || '本机未登录 cbhcli');
...
export function streamChat(payload, onEvent, onError, onDone, signal) {
    ...
    ServerConnection.makeRequest(apiUrl('chat'), {
        ...
        if (!response.ok) {
            ...
            if (response.status === 401 && code === 'not_logged_in') {
                const authErr = new Error(serverMessage || '本机未登录 cbhcli');
                authErr.notLoggedIn = true;
                throw authErr;
            }
        ...
        if (abortedByUs ||
            (err === null || err === void 0 ? void 0 : err.name) === 'AbortError' ||
            /signal is aborted|user aborted/i.test(String((err === null || err === void 0 ? void 0 : err.message) || ''))) {
            return;
        }
