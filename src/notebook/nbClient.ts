...
function ok(output: string, data: any = {}) {
...
export class NotebookClient {
  ...
  getCurrentSelectionContext(): SelectionContext | null {
    try {
      ...
      const cw = this._app.shell.currentWidget as any;
  ...
  private _isFileEditorWidget(w: any): boolean {
    if (!w) {
      return false;
    }
    ...
    const ed = w.content?.editor ?? w.editor;
    return (
      typeof w.context?.path === 'string' &&
      !!ed &&
  ...
  private _cmEditorOf(w: any): any {
    return w?.content?.editor ?? w?.editor ?? null;
  }

  /** 由 CodeMirror 选区（0-based line/column）抽取文本 + 行列号（1-based）。 */
  private _extractSelFromCm(
    editor: any
  ): { text: string; startLine: number; endLine: number; startCol: number; endCol: number } | null {
    try {
      if (!editor || typeof editor.getSelection !== 'function') {
        return null;
      }
      const sel = editor.getSelection() as any;
      ...
      if (sel.start.line === sel.end.line) {
        ...
        const parts: string[] = [(lines[sel.start.line] || '').slice(sel.start.column)];
  ...
  private _extractCursorFromCm(
    editor: any
  ): { line: number; col: number; lineText: string } | null {
    try {
      if (!editor || typeof editor.getSelection !== 'function') {
        return null;
      }
  ...
  private _isNbEditMode(nb: NotebookPanel): boolean {
    try {
      const mode = (nb.content as any).mode;
      if (mode === 'edit') {
        return true;
      }
      // 兜底：活动 cell 编辑器聚焦也算编辑态
      const ac: any = nb.content.activeCell;
      return !!ac?.editor?.hasFocus?.();
    } catch {
      return false;
    }
  }
  ...
  private _cellCursorCtx(nb: NotebookPanel, cellWidget: any): SelectionContext | null {
    const info = this._extractCursorFromCm(cellWidget?.editor);
    if (!info) {
      return null;
    }
  ...
  /** 文件编辑器的光标位置上下文（空选区）。 */
  private _fileEditorCursorCtx(w: any): SelectionContext | null {
  ...
  private _cellTextSelectionCtx(nb: NotebookPanel, cellWidget: any): SelectionContext | null {
    const info = this._extractSelFromCm(cellWidget?.editor);
    if (!info) {
      return null;
    }
    const cellIndex = this._cellIndexOf(nb, cellWidget);
  ...
  /** 文件编辑器的文本选区上下文（含列号）。 */
  private _fileEditorSelectionCtx(w: any): SelectionContext | null {
  ...
  private _anyFileEditorSelection(): SelectionContext | null {
    try {
  ...
  private _anyEditorTextSelection(): SelectionContext | null {
    try {
      const cmContents = document.querySelectorAll('.cm-content');
      for (const ct of Array.from(cmContents) as any[]) {
        const view = ct.cmView?.view ?? ct.cmView;
        if (!view?.state) {
          continue;
        }
        ...
        const editorRoot = ct.closest('.cm-editor') as HTMLElement | null;
        const cellNode = editorRoot?.closest('.jp-Cell') as HTMLElement | null;
        if (cellNode) {
          const nb = this._currentNotebook();
          if (nb) {
            let cellIndex = -1;
            nb.content.widgets.forEach((w: any, i: number) => {
  ...
  private _findPathByNode(node: HTMLElement | null): string {
    if (!node) {
      return '';
    }
    try {
      for (const w of Array.from(this._app.shell.widgets('main'))) {
        const we = w as any;
        if (we.node && we.node.contains(node) && typeof we.context?.path === 'string') {
  ...
  private _notebookSelectedCells(nb: NotebookPanel): SelectionContext | null {
    ...
    if (selectedIndexes.length === 0) {
      nb.content.widgets.forEach((cell: any, i: number) => {
        let sel = false;
        try {
    ...
    if (useIndexes.length === 0) {
      ...
      const activeEmpty =
        ...
        String(model.cells.get(activeIdx)?.sharedModel?.getSource?.() ?? '').trim() === '';
      if (!activeEmpty) {
        return null;
      }
    ...
    for (const idx of useIndexes) {
      parts.push(String(model.cells.get(idx)?.sharedModel?.getSource?.() ?? ''));
    }
    const loc =
      useIndexes.length === 1
  ...
  private async _runTask(task: { task_id: string; action: string; params: any }): Promise<void> {
    let result: any;
    try {
  ...
  private _currentNotebook(): NotebookPanel | null {
  ...
  private _findCell(nb: NotebookPanel, params: any): { cell: any; index: number } | null {
    const model = nb.content.model;
    if (!model) {
      return null;
    }
  ...
  private _cellIndexOf(nb: NotebookPanel, cell: any): number {
    ...
    const cells = nb.content.model?.cells;
    if (!cells) {
      return -1;
    }
    for (let i = 0; i < cells.length; i++) {
      if (cells.get(i) === cell || cells.get(i) === cell?.model) {
        return i;
      }
    }
    return -1;
  ...
  private _getSelection(params: any): any {
    const nb = this._currentNotebook();
    if (!nb) {
    ...
    const data: any = {
      ...
      kernel: nb.context.sessionContext.session?.kernel?.name || null,
      cells: []
    };
  ...
  private _getEditorSelection(cell: any): string | null {
    try {
  ...
  private _listCells(params: any): any {
    const nb = this._currentNotebook();
    if (!nb) {
      return fail('当前没有打开的 notebook（请先打开 .ipynb 文件）');
    }
    ...
    for (let i = 0; i < cellModels.length; i++) {
      ...
      if (includeOutputs && cm.type === 'code') {
        const widget = nb.content.widgets[i] as CodeCell | undefined;
        if (widget) {
          const outs: string[] = [];
          for (const o of widget.outputArea.model.toJSON()) {
            if (o.output_type === 'stream') {
              ...
              const mime = (o as any).data as any;
              outs.push(String(mime?.['text/plain'] || '').slice(0, maxOut));
   ...
   */
  private _insertAtCursor(source: string, newCode: string, line: number, col: number): string {
    const lines = source.split('\n');
  ...
  private async _editCell(params: any): Promise<any> {
    const nb = this._currentNotebook();
    if (!nb) {
      return fail('当前没有打开的 notebook（请先打开 .ipynb 文件）');
    }
    ...
    if (params.selection_text !== undefined && params.selection_text !== null) {
      ...
      if (selRange && selRange.text === target) {
        cell.model.sharedModel.setSource(
          current.slice(0, selRange.start) + newCode + current.slice(selRange.end)
        );
      } else {
        ...
        cell.model.sharedModel.setSource(
          current.slice(0, pos) + newCode + current.slice(pos + target.length)
        );
      }
    } else {
      // 整体替换
  ...
  private _cellSelectionRange(cell: any): {
    text: string;
    start: number;
    end: number;
  } | null {
  ...
  private async _insertCell(params: any): Promise<any> {
    const nb = this._currentNotebook();
    if (!nb) {
      return fail('当前没有打开的 notebook（请先打开 .ipynb 文件）');
    }
  ...
  private async _deleteCell(params: any): Promise<any> {
    const nb = this._currentNotebook();
    if (!nb) {
      return fail('当前没有打开的 notebook（请先打开 .ipynb 文件）');
    }
  ...
  private async _executeCell(params: any): Promise<any> {
    const nb = this._currentNotebook();
    if (!nb) {
      return fail('当前没有打开的 notebook（请先打开 .ipynb 文件）');
    }
  ...
  private async _executeCellInPlace(
    nb: NotebookPanel,
    cell: CodeCell,
    code: string,
    timeoutMs: number
  ...
  private _extractCellOutputs(cell: CodeCell): {
    outputs: string[];
    errorText: string;
    resultText: string;
  } {
  ...
  private _executeCode(
    ...
    code: string,
    timeoutMs: number,
    syncCell: boolean = false
  ): Promise<any> {
    return new Promise(resolve => {
      const kernel = nb.context.sessionContext.session?.kernel;
      if (!kernel) {
      ...
      future.onIOPub = msg => {
        ...
        try {
          if (type === 'stream') {
            ...
            const txt = (d as any)['text/plain'] !== undefined ? String((d as any)['text/plain']) : '';
      ...
      future.done
        ...
        .catch((err: any) => {
          window.clearTimeout(timer);
          resolve(fail(`内核执行失败: ${err?.message || String(err)}`));
        });
    });
  }
  ...
  private _fileRead(params: any): any {
    const editor = this._findFileEditor(params.path, params.filename);
    if (!editor) {
      return fail(
        `未找到打开的文件${params.filename ? ` '${params.filename}'` : ` '${params.path}'`}。` +
  ...
  private async _fileEdit(params: any): Promise<any> {
    const editor = this._findFileEditor(params.path, params.filename);
    if (!editor) {
      return fail(
        `未找到打开的文件${params.filename ? ` '${params.filename}'` : ` '${params.path}'`}。` +
    ...
    const sharedModel = cmEditor?.model?.sharedModel;
    if (!sharedModel || typeof sharedModel.setSource !== 'function') {
      return fail('无法访问文件编辑器内容');
    }
    const source = String(sharedModel.getSource() ?? '');
    let newText: string | null = null;
    let desc = '';

    // 在光标处插入新行（不替换现有内容）
    if (params.insert_at_line !== undefined && params.insert_at_line !== null) {
      const insertStr = String(params.new_str ?? '');
      const line = parseInt(String(params.insert_at_line), 10);
      const col = params.insert_at_col !== undefined ? parseInt(String(params.insert_at_col), 10) : 1;
      sharedModel.setSource(this._insertAtCursor(source, insertStr, line, col));
      return ok(
        `已在文件 ${editor.context.path} 第 ${line} 行${col > 1 ? '下方' : '上方'}插入内容，` +
        '记得在编辑器中保存（Ctrl+S）'
      );
    }
    ...
    if (params.content !== undefined) {
      ...
      const newText2 =
        source.slice(0, pos) + String(params.new_str ?? '') + source.slice(pos + target.length);
   ...
   */
  private _findFileEditor(path?: string, filename?: string): any | null {
    let found: any = null;
    let foundScore = -1;
  ...
  private _fileEditorSource(w: any): string {
    const ed = this._cmEditorOf(w);
    try {
      return String(ed?.model?.sharedModel?.getSource?.() ?? '');
    } catch {
      return '';
    }
  }
